...
def generate_executive_pdf(project_name: str, status_label: str, status_message: str,
    ...
    pdf.set_font("Helvetica", "B", 18)
    pdf.set_xy(15, 8)
    pdf.cell(120, 10, "NEXUS - Executive Governance Report", ln=False)
    ...
    pdf.cell(0, 12, "Generated by Nexus Governance Intelligence  |  Confidential -- For Steering Committee Use Only", align="C")
    ...
    with col_report:
        ...
        if st.button("📄 Export Report", type="primary", use_container_width=True):
            try:
                ...
                file_name = f"nexus_report_{selected_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
                st.download_button(
...
def render_analysis_page(supabase_client: Client) -> None:
    ...
    st.title("Nexus -- Governance Intelligence")
    st.markdown(
            ...
            if not env_path.exists() or not api_key:
                ...
                if not has_cached_results:
                    ...
                    try:
                        ...
                        with st.spinner("🔍 Nexus is scanning your documents -- this may take 20-40 seconds..."):
                        ...
                        try:
                            ...
                            st.success("✅ Analysis saved to Nexus database -- Project: " + project_name)
...
st.set_page_config(
    page_title="Nexus -- Governance Intelligence",
    page_icon="◆",
    layout="wide",
